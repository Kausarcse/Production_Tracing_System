...
mysqli_query($con,"START TRANSACTION") or die(mysqli_error($con));
	...
	//$select_sql_for_duplicacy_for_transaction_table="select * from `travel_card_details_for_transactions` where `travel_card_id`='$travel_card_id' and process_id='$process_id' and process_name='$process_name' and process_quantity='$process_quantity'";
 ...
 if($result)
 {
 		...
 		$update_sql_statement2="UPDATE `travel_card_details` SET `actual_ratio`='$actual_ratio',`actual_roll_size`='$actual_roll_size',`recording_person_id`='$user_id',`recording_person_name`='$user_name',`recording_time`=NOW() WHERE `travel_card_id`='$travel_card_id'";

		mysqli_query($con,$update_sql_statement2) or die(mysqli_error($con));
 }
