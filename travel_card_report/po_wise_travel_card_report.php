...
// alert(url_encoded_form_data);
             ...
             $.ajax({
                    url: 'travel_card_report/po_wise_travel_card_summary_data_table.php',
                    dataType: 'text',
                    type: 'post',
            ...
            <form class="form-horizontal" action="" method="POST" name="po_wise_summary" id="po_wise_summary">
                ...
                <table class="table table-bordered">
                 ...
                 <div class="form-group form-group-sm" id="form-group_for_feom_date" style="margin-right:0px; color:#00008B;">
                    ...
                    <label class="control-label col-sm-4" for="process" style="margin-right:0px; color:#00008B;">PO Number. :</label>
                    <div class="col-sm-5">
                       

                        <select  class="form-control for_auto_complete_po" id="po_number" name="po_number" >
                                    <option select="selected" value="select">Select PO</option>
                                    <?php 
                                          $sql = 'select * from `po_creation`';
                                          $result= mysqli_query($con,$sql) or die(mysqli_error($con));
                                          while( $row = mysqli_fetch_array( $result))
                                          {
                                          
                                              echo '<option value="'.$row['po_id'].'?fs?'.$row['po_number'].'">'.$row['po_number'].'</option>';
